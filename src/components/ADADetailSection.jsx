...
const FIELDS = [
  { key: "ada_parking",            labelKey: "adaParking",   emoji: "🚗", fallback: "Accessible Parking"  },
  { key: "ada_entrance_ramp",      labelKey: "adaEntrance",  emoji: "♿", fallback: "Accessible Entrance" },
  { key: "ada_restroom",           labelKey: "adaRestroom",  emoji: "🚻", fallback: "Accessible Restroom" },
  { key: "ada_accessible_seating", labelKey: "adaSeating",   emoji: "🪑", fallback: "Accessible Seating"  },
];

const COMPLIANCE_CONFIG = {
  accessible:           { label: "Accessible",           labelKey: "adaAccessible",    bg: "bg-emerald-100", text: "text-emerald-800", border: "border-emerald-200" },
  partially_accessible: { label: "Partially Accessible", labelKey: "adaPartial",       bg: "bg-yellow-100",  text: "text-yellow-800",  border: "border-yellow-200" },
  not_accessible:       { label: "Not Accessible",       labelKey: "adaNotAccessible", bg: "bg-red-100",     text: "text-red-800",     border: "border-red-200" },
  unknown:              { label: "Unknown",               labelKey: "adaUnknown",       bg: "bg-slate-100",   text: "text-slate-600",   border: "border-slate-200" },
};
  ...
  if (value === "yes") {
    return (
      <span className="flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full whitespace-nowrap">
        <CheckCircle2 className="w-3 h-3" /> {t?.yes || "Yes"}
      </span>
    );
  }
  if (value === "no") {
    return (
      <span className="flex items-center gap-1 text-xs font-bold text-red-700 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full whitespace-nowrap">
        <XCircle className="w-3 h-3" /> {t?.no || "No"}
      </span>
    );
  }
  return (
    <span className="flex items-center gap-1 text-xs font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded-full whitespace-nowrap">
      <HelpCircle className="w-3 h-3" /> {t?.unknown || "Unknown"}
    </span>
  );
}
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-extrabold text-slate-900 tracking-tight">{t?.adaAccessibility || "ADA Accessibility"}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {FIELDS.map(({ key, labelKey, emoji, fallback }) => (
          <div key={key} className="flex items-center justify-between gap-2 px-3 py-2.5 bg-slate-50 rounded-xl border border-slate-100">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs font-semibold text-slate-700 leading-tight">{langCode && t?.[labelKey] ? t[labelKey] : fallback}</span>
            </div>
            <StatusBadge value={adaData[key] || "unknown"} t={t} />
          </div>
        ))}
      </div>
      ...
      <p className="text-[11px] text-slate-400 mt-3 leading-relaxed italic">
        {t?.adaDataSource || "Accessibility data sourced from Google Maps."}{lastUpdatedStr ? ` ${t?.lastUpdated || "Last updated"}: ${lastUpdatedStr}.` : ""}
      </p>
    </div>
  );
}