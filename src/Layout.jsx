...
export default function Layout({ children, currentPageName }) {
  return (
    <div className="min-h-screen flex flex-col">
      {/* Navigation */}
      <nav className="bg-slate-900 border-b-4 border-[#4CAF50] sticky top-0 z-50" style={{ fontFamily: "Nunito, sans-serif" }}>
        <div className="max-w-5xl mx-auto px-4 py-2.5">
          <div className="flex items-center justify-between gap-2">
            {/* Logo — big & fun */}
            <a href="/" className="flex items-center gap-2 hover:scale-105 transition-transform flex-shrink-0">
              <div className="w-9 h-9 rounded-2xl bg-[#4CAF50] flex items-center justify-center shadow-md border-2 border-white/20">
                <ShieldCheck className="w-5 h-5 text-white" />
              </div>
              <div>
                <span className="font-black text-xl text-white tracking-tight leading-none">SafeEats</span>
                <div className="text-[9px] text-[#81c784] font-bold leading-none">🛡️ Food Safety for Everyone</div>
              </div>
            </a>
            {/* Nav links */}
            <div className="flex items-center gap-0.5 flex-wrap justify-end">
              <Link to="/dashboard"><button className={navPill}><BarChart2 className="w-4 h-4" />📊 Stats</button></Link>
              <Link to={createPageUrl("About")}><button className={navPill}><Info className="w-4 h-4" />ℹ️ About</button></Link>
