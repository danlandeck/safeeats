...
function shapeDate(attrs: Record<string, unknown>): Record<string, unknown> {
...
async function query(layer: number, where: string, count: number): Promise<Record<string, unknown>[]> {
  const rows = await arcgisFeatureQuery(`${SERVICE}/${layer}`, where, count, 'Inspection_Date DESC');
  return rows.map(shapeDate);
}
